...
Merchant responses are untrusted: normalization tags every merchant-derived
value with provenance/taint. Before the buyer agent proposes a purchase, the
capability firewall enforces `payment.propose`; the privileged `payment.execute`
capability is denied, so the (future) executor is unreachable from here.
...
class Orchestrator:
    def __init__(self, merchants: list[MerchantAgent] | None = None) -> None:
    ...
    async def run(self, session: AsyncSession, request: CreateMissionRequest) -> Mission:
        ...
        await self._transition(
            ...
            payload={"merchants": [m.merchant_id for m in self.merchants]},
        )

        # Query merchants (untrusted responses)
        raw_offers = []
        for m in self.merchants:
            raw_offers.extend(m.quote(c, request.quantity))
        ...
        await self._transition(
            ...
            payload={"raw_offer_count": len(raw_offers)},
        )

        # RUNTIME AUTHORITY ENFORCEMENT.
        # User policy is held at USER_SIGNED_POLICY authority. Merchants may try
        ...
        for raw in raw_offers:
            for field, attempted in raw.claims.items():
                if not register.is_protected(field):
                    continue
                incoming = untrusted(attempted, source=f"merchant:{raw.merchant_id}")
                try:
        ...
        # Protected values are unchanged by merchant claims.
        assert register.get("hard_limit_inr").value == c.hard_limit_inr

        # Normalize -> coupled ProvenancedOffer (description dropped at ingress).
        normalized: list[ProvenancedOffer] = normalize_offers(raw_offers, c)
